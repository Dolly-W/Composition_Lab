...
class BankAccount {
    ...
    virtual void deposit(double amount) {
        if (amount > 0) {
            ...
            transactionHistory.push_back(Transaction{"Deposit", amount, "Time"});
        }
        else {
    ...
    void printHistory() {
        if (transactionHistory.empty()) {
            std::cout << "There has been no Transactions yet" << std::endl;
            return;
        }
        std::cout << "Transaction History for Account- " << accountNumber << ": " << std::endl;
        for (int i = 0; i < transactionHistory.size(); i++) {
            std::cout << transactionHistory[i].type << " of $" << transactionHistory[i].amount << " at " << transactionHistory[i].timestamp << std::endl;
        }
    }
    ...
    virtual void withdraw(double amount) {
        if (amount > 0 && amount <= balance) {
            ...
            transactionHistory.push_back(Transaction{"Withdrawal", amount, "Time"});
        }
        else {
...
private:
    ...
    double balance;
    std::vector<Transaction> transactionHistory; //store transactions
};
