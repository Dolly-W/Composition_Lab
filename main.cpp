...
int main() {
    ...
    do {
        ...
        switch (choice) {
            case 1: {
                ...
                if (choice == 1) {
                    accounts.push_back(
                        new CheckingAccount(base.GetAccountNumber(), base.GetHolderName(), base.GetBalance()));
                    std::cout << "Checking Account Has Been Successfully Created. Note: Transaction Fee: $1.50" <<
                            std::endl;
                } else if (choice == 2) {
                    accounts.push_back(new SavingAccount(base.GetAccountNumber(), base.GetHolderName(),
                                                         base.GetBalance()));
            ...
            case 2: {
                ...
                while (true) {
                    ...
                    for (int i = 0; i < accounts.size(); i++) {
                        std::cout << i + 1 << ") " << accounts[i]->GetAccountNumber() << " - " << accounts[i]->
                                GetHolderName() << std::endl;
                    }
            ...
            case 5: {
                ...
                if (*accounts[acc1 - 1] == *accounts[acc2 - 1]) {
                    ...
                    std::cout << "Account " << acc1 << " and " << acc2 << " have different account numbers" <<
                            std::endl;
                }
    ...
    //-Testing transaction-
    std::cout << "~~~Executing Test Accounts..." << std::endl;
    //Test accounts
    CheckingAccount testChecking("1234", "Gerard", 1000);
    SavingAccount testSaving("5678", "Geoff", 500);

    //Test deposit and withdrawal transactions
    testChecking.deposit(400);
    testSaving.withdraw(200);
    testSaving.deposit(100);
    testSaving.withdraw(400);

    std::cout << "~~Test: Transaction History~~" << std::endl;

    //Print histories
    std::cout << "Checking Account History:" << std::endl;
    testChecking.printHistory();

    std::cout << "Saving Account History:" << std::endl;
    testSaving.printHistory();
